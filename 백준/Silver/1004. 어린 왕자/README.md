...
---
 ...
 ### 풀이
 ---
 + 첫 시도: 점 사이의 거리를 구할 때 <cmath>라이브러리의 sqrt()함수를 사용했다 -> 부동소수점 때문에 정확한 거리 측정 불가로 실패

 + 해결책: 거리의 재곱과 반지름의 제곱을 비교 -> 성공

 + 문제점: 1. 코드가 너무 장황하고 지저분한 것 같다. 2. 문제에 대한 오해 - 출력을 한번에 하지 않아도 되기 때문에 (동적할당으로) 출력값을 저장해놓을 필요가 없다. 
